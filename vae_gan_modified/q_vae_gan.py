''' Query-to-image VAE-GAN with Modified Discriminator
NABUS, Martin Roy
...
label_shape = (40,)
latent_dim = 128
epochs = 3000	# Treats one epoch as a per-minibatch run instead of one pass on all data
epsilon_std = 1.0
kernel_size = 5
filter = 32
gamma = 0.75
...
x = Dense(64*64, activation='tanh')(input_labels)
x = Reshape((64,64,1))(x)
for i in range(3) :
	filter *= 2 # 64, 128, 256
...
# decoder.summary()
# plot_model(decoder, to_file='qvg_decoder.png', show_shapes=True)
...
outputs = decoder(encoder(input_labels)[2])
vae_model = Model(input_labels, outputs)
...
if(len(sys.argv) == 1 or (len(sys.argv) > 1 and sys.argv[1] == 'continue')) :
	if(len(sys.argv) > 1 and sys.argv[1] == 'continue') :
		encoder.load_weights('q_vae_gan_enc.h5')
		decoder.load_weights('q_vae_gan_dec.h5')
		discriminator.load_weights('q_vae_gan_disc.h5')
	...
	if(preload) : # X_train will all ALL images
		X_train = []; ctr = 0
	...
	for epoch in range(epochs) :
		...
		disc_loss1 = discriminator_model.train_on_batch(imgs, np.ones((half_batch,1)))
		lcode = encoder.predict(lbls)[2]
		lc_img = decoder.predict(lcode)
		disc_loss2 = discriminator_model.train_on_batch(lc_img, np.zeros((half_batch,1)))
		noise = np.random.normal(0, 1, (half_batch, latent_dim))
		gen_imgs = decoder.predict(noise)
		disc_loss3 = discriminator_model.train_on_batch(gen_imgs, np.zeros((half_batch,1)))
		...
		if(preload) : 
			...
			idx = idx-1; 
		lbls = X_labels[idx]
		latent_pred = encoder.predict(lbls)[2]
		...
		if(preload) : 
			...
			idx = idx-1; 
		lbls = X_labels[idx]
		enc_loss = vae_model.train_on_batch(lbls, imgs)
		# checkpointing
		print("EPOCH",epoch+1)
		...
		if((epoch+1)%chkpt==0 and epoch < epochs) :
			...
			encoder.save_weights('q_vae_gan_enc.h5')
			decoder.save_weights('q_vae_gan_dec.h5')
			discriminator.save_weights('q_vae_gan_disc.h5')
	...
	encoder.load_weights('q_vae_gan_enc.h5')
	decoder.load_weights('q_vae_gan_dec.h5')
	vect = input("Input attribute vector or q to quit... ")
	if(vect != 'q') : # LOAD 
		...
		X_labels = np.asarray(X_labels)
	while(vect != 'q') :
		avec = np.asarray([float(x) for x in vect.split()])
		# Find images in training set that have the same attribute vector as input; for comparison purposes only
		img_idxs = np.where((X_labels == avec).all(axis=1))	
		if any(map(len, img_idxs)) :
			print(len(img_idxs[0]),"images with same attributes found in training set")
			img_idx = np.random.choice(img_idxs[0])
			name = str(img_idx+1)
			while(len(name) < 6) : name = '0' + name
			name = name + '.jpg'
			...
			print("No image in training set with specified attributes")
			avecs = []; avecs.append(avec); avecs = np.asarray(avecs)
			lcode = encoder.predict(avecs)[2]
			res = decoder.predict(lcode)
			res = np.uint8(res*127.5 + 127.5) 
